...
public class TaskRepository {
    ...
    public List<TaskEntity> findAllByEmail(String email) {
        ...
        String query = "SELECT t.id, t.name AS task_name, j.name AS job_name, u.fullname AS user_name, "
                + "t.start_date, t.end_date, s.name AS status "
                + "FROM users u "
                + "JOIN tasks t ON u.id = t.user_id "
                + "JOIN jobs j ON j.id = t.job_id "
                + "JOIN status s ON s.id = t.status_id "
        ...
        try(Connection connection = MysqlConfig.getConnection();
            ...
            System.out.println("findAllByEmail : " + e.getMessage());
        }
        return tasks;
    }
    ...
    public TaskEntity findById(int id) {
        String query = "SELECT "
                + "j.name AS job_name, t.name AS task_name, t.start_date, t.end_date, u.fullname, t.status_id "
                + "FROM tasks t "
                + "JOIN jobs j ON j.id = t.job_id "
                + "JOIN status s ON s.id = t.status_id "
        ...
        PreparedStatement statement = connection.prepareStatement(query)){
            ...
            while (result.next()) {
                ...
                task.setStart_date(result.getDate("start_date"));
                task.setEnd_date(result.getDate("end_date"));
                task.setStatus_id(result.getInt("status_id"));
                task.setUser_name(result.getString("fullname"));
            }
        }catch (Exception e){
